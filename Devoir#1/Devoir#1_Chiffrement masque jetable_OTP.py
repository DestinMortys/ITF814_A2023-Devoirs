...
def E2(k, m):
    ...
    for i in range(len(m)):
        # XOR entre la clé et le message
        encrypted_bit = str(int(k[i]) ^ int(m[i]))
...
def D2(k, c):
    if len(k) != len(c):
        raise ValueError(
            "La clé et le cryptogramme doivent avoir la même longueur")

    plaintext = ""
    for i in range(len(c)):
        # XOR entre la clé et le cryptogramme
        decrypted_bit = str(int(k[i]) ^ int(c[i]))
...
for i in range(3):
    ...
    plaintext = ''.join(chr(int(plaintext_bits[i:i+8], 2))
                        for i in range(0, len(plaintext_bits), 8))  # Conversion en texte
